...
#### 소스코드 준비
- [ ] VINS-Fusion 저장소 클론 (`HKUST-Aerial-Robotics/VINS-Fusion`)
- [ ] 의존성 패키지 설치
...
#### EuRoC 데이터셋 다운로드
- [ ] EuRoC MAV Dataset 웹사이트 접속
- [ ] `MH_01_easy` 시퀀스 다운로드 (가장 쉬운 시퀀스)
- [ ] ROS bag 형식 선택 권장
- [ ] 데이터 압축 해제 및 경로 확인
...
- [ ] 카메라 이미지 토픽 확인
- [ ] IMU 데이터 토픽 확인
- [ ] Ground truth 데이터 확인 (`state_groundtruth_estimate0`)
- [ ] 캘리브레이션 파일 확인

### 2.2 VINS-Fusion 실행

#### 설정 파일 확인
- [ ] EuRoC용 config 파일 찾기 (`config/euroc/`)
- [ ] 카메라 파라미터 확인
- [ ] IMU 파라미터 확인
...
- [ ] VINS-Fusion 노드 실행
- [ ] RViz 시각화 실행
- [ ] 데이터셋 재생 (`rosbag play MH_01_easy.bag`)

#### 결과 확인
- [ ] 궤적이 RViz에 표시되는지 확인
- [ ] 특징점 트래킹 시각화 확인
- [ ] VINS 추정 궤적 저장 (TUM 또는 EuRoC 형식)

### 2.3 evo로 첫 평가

#### 궤적 비교
- [ ] Ground truth와 추정 궤적 형식 맞추기
- [ ] ATE 계산: `evo_ape euroc gt.csv estimate.csv -va --plot`
- [ ] 결과 스크린샷 저장
- [ ] 대략적인 오차 수준 파악 (숫자 자체보다 "돌아간다"가 중요)
...
#### 관찰하면서 질문 작성
- [ ] Feature Tracker 창에서 점들이 뭘 의미하는지?
- [ ] IMU data가 어떻게 Vision과 합쳐지는지?
- [ ] Loop closure가 뭔지?
- [ ] Marginalization이 뭔지?
- [ ] Pre-integration이 뭔지?
- [ ] Factor graph가 뭔지?
- [ ] ATE, RPE가 정확히 뭘 측정하는지?

#### 코드 훑어보면서 질문 작성
- [ ] `Eigen::Quaterniond`가 뭔지?
- [ ] `SE3`, `SO3`가 뭔지?
- [ ] `ceres::Problem`이 뭔지?
- [ ] `imu_factor`가 뭘 하는지?
...
### 실행
- [ ] VINS-Fusion + EuRoC 실행 성공
- [ ] RViz에서 궤적 확인
- [ ] evo로 Ground truth 비교 실행

### 학습 준비
- [ ] **"모르는 개념 목록"** 작성 완료 (최소 10개)
- [ ] 우선순위 정하기 (가장 궁금한 것부터)
