...
위 다이어그램의 각 요소가 의미하는 것:

1. **에피폴라 평면** (상단 삼각형 영역)
   - **카메라1 중심** + **3D 점 P** + **카메라2 중심**을 포함하는 하나의 평면
   - 이 세 점이 항상 같은 평면 위에 있음
...
3. **카메라1과 카메라2**
   - 두 카메라의 광학 중심 (점선으로 연결됨)
   - 이 거리를 **베이스라인(Baseline)** 이라고 함

4. **p₁과 p₂** (이미지 평면 위의 점)
   - **p₁**: 3D 점 P가 카메라1 이미지에 투영된 2D 위치
   - **p₂**: 3D 점 P가 카메라2 이미지에 투영된 2D 위치
...
카메라1에서 P로 향하는 광선 ──┐
                              │  이 두 광선과
카메라2에서 P로 향하는 광선 ──┘  베이스라인이
                                 
        → 모두 하나의 평면(에피폴라 평면) 위에 있음! ✨
```
...
- E는 이 기하학적 제약을 3×3 행렬로 수학적으로 표현

**SLAM에서의 활용:**
- 대응점 탐색 범위 축소 (2D 영역 → 1D 라인)
- 잘못된 매칭 검출
- 카메라 포즈(R, t) 추정
...
> [!NOTE]
> **핵심 개념**: 3차원 공간에서 (일직선상에 있지 않은) **세 점은 항상 정확히 하나의 평면을 정의**합니다.

##### 다각도 시각화

![에피폴라 평면 - 여러 시점](images/epipolar_plane_views.png)

**각 뷰의 의미:**

1. **3D 원근 뷰 (상단)**
   - 카메라1(C1), 카메라2(C2), 3D 점 P가 만드는 삼각형
   - 파란색 반투명 영역 = **에피폴라 평면** (무한히 
펼쳐진 평평한 표면)
   - 점선 = 세 점을 연결하는 선들
...
3. **옆에서 본 모습 (우측 하단)**
   - XZ 평면에서 측면 뷰
   - 평면이 **대각선으로 기울어진** 것을 확인

##### 평면의 방향은 P의 위치에 따라 변함!

![에피폴라 평면 - 위치별 변화](images/epipolar_plane_positions.png)

**중요한 통찰:**

| P의 위치 | 평면의 방향 | 예시 |
|---------|-----------|------|
| **위쪽** | 위로 기울어짐 | 드론이 카메라들 위를 날아갈 때 |
| **옆쪽** | 수직으로 세워짐 | 사람이 카메라 옆을 지나갈 때 |
| **앞쪽** | 앞으로 기울어짐 | 자동차가 카메라 정면에서 접근할 때 |

**항상 변하지 않는 것:**
- **베이스라인** (C1-C2 연결선)은 항상 평면에 포함됨
- 두 카메라 위치는 고정
...
문은 경첩을 중심으로 여러 각도로 열릴 수 있듯이,
에피폴라 평면도 베이스라인을 축으로 
P의 위치에 따라 다양한 각도로 "회전"합니다.
```
...
Essential Matrix를 `E = U × Σ × Vᵀ` 형태로 분해합니다.
...
```python
# 예시
S = [1.0, 1.0, 0.0]  # 이상적인 경우
S = [1.2, 0.9, 0.05]  # 실제 (노이즈 포함)
```

> [!NOTE]
> 실제 데이터에서는 노이즈로 인해 정확히 `[σ, σ, 0]` 형태가 아닐 수 있습니다. 이 경우 특이값을 정규화하여 `[1, 1, 0]`으로 만들어야 합니다.
...
**W 행렬의 의미:**
- Z축 기준으로 **+90도 회전**을 나타내는 회전 행렬
- Essential Matrix의 구조적 특성상, R을 추출하려면 이 특수 행렬이 필요

**왜 W가 필요한가?**

Essential Matrix `E = [t]ₓ R`의 구조에서:
- `[t]ₓ`: t의 외적 행렬 (skew-symmetric)
- `R`: 회전 행렬

SVD 분해 후 R과 t를 복원하려면 이 구조를 역으로 풀어야 하는데, 그 과정에서 W 행렬이 등장합니다.
...
[t]ₓ는 다음과 같이 분해 가능:
[t]ₓ = U Z Uᵀ  (Z는 특정 형태의 행렬)

이때 Z = diag(1, 1, 0) W  또는  Z = diag(1, 1, 0) Wᵀ
```
...
**두 가지 R이 나오는 이유:**
- W를 사용: 한 방향의 회전
- Wᵀ를 사용: 반대 방향의 회전 (-90도)

**행렬식(Determinant) 확인:**

올바른 회전 행렬은 **det(R) = +1**이어야 합니다.

```python
if np.linalg.det(R1) < 0:
    R1 = -R1  # 반사 변환(reflection) → 회전으로 수정
```
...
Essential Matrix `E = [t]ₓ R`에서:
- E의 null space(영공간)가 바로 t 방향
- SVD 후, **가장 작은 특이값(=0)에 대응하는 U의 열벡터**가 null space

```python
# 검증
E @ t ≈ 0  # t는 E의 null space에 속함
```

**방향만 알 수 있는 이유:**
- Essential Matrix는 **스케일 불변(scale-ambiguous)**
- 실제 이동 거리를 알 수 없고, **방향만** 알 수 있음
- 예: 1m 이동과 10m 이동이 동일한 E를 생성
...
- R: 2가지 가능 (시계/반시계 방향 회전)
- t: 2가지 가능 (+방향/-방향)
- 총 4가지 기하학적 배치가 수학적으로 가능

---

#### 6️⃣ Cheirality Check (키랄리티 체크)

> [!IMPORTANT]
> **필수 단계**: 4개의 후보 중 **올바른 해 1개**를 선택해야 합니다!

**방법: 삼각측량(Triangulation) 후 깊이 확인**

```python
def select_valid_solution(candidates, pts1, pts2, K):
    """Cheirality check로 올바른 (R, t) 선택"""
    for R, t in candidates:
        # 3D 점 복원 (삼각측량)
        X = triangulate(pts1, pts2, K, R, t)
        
        # 두 카메라에서 깊이 확인
        depth1 = X[:, 2]  # 카메라1 기준 Z 좌표
        depth2 = (R @ X.T + t.reshape(3, 1))[2, :]  # 카메라2 기준
        
        # 모든 점이 두 카메라 앞에 있는지 확인
...
**올바른 해의 조건:**
- 복원된 3D 점이 **두 카메라 모두 앞**에 있어야 함 (양수 깊이)
- 4개 중 일반적으로 **1개만** 이 조건을 만족
...
def decompose_essential_matrix(E):
    """Essential Matrix에서 R, t 추출 (전체 과정)"""
    # 1. SVD 분해
    U, S, Vt = np.linalg.svd(E)
...
print("4가지 후보 (R, t):")
for i, (R, t) in enumerate(candidates, 1):
    print(f"\n후보 {i}:")
    print(f"R:\n{R}")
    print(f"det(R) = {np.linalg.det(R):.4f}")
    print(f"t: {t}")

# Cheirality check로 최종 선택
# (실제로는 대응점 필요)
# R_final, t_final = select_valid_solution(candidates, pts1, pts2, K)
```
...
| 1 | SVD 분해 | U, Σ, Vᵀ |
| 2 | W 행렬 준비 | 90도 회전 행렬 |
| 3 | R 추출 | U·W·Vᵀ, U·Wᵀ·Vᵀ (2개) |
| 4 | t 추출 | U[:, 2], -U[:, 2] (2개) |
| 5 | 조합 생성 | 2×2 = 4가지 후보 |
| 6 | Cheirality | 올바른 1개 선택 ✅ |

> [!CAUTION]
> **필수 검증 사항:**
> - SVD 전 E가 올바른 Essential Matrix인지 확인 (랭크 2)
> - det(R) = +1 확인
> - Cheirality check 필수 수행
