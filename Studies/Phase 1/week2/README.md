...
> [!question]
> **Q: 고유값 분해를 하는 가장 큰 이유가 공분산 때문인가요?**
>
> **네, 맞습니다.** SLAM이나 데이터 분석에서 고유값 분해를 수행하는 가장 핵심적인 이유는 **데이터의 분포(공분산)를 기하학적으로 해석하기 위해서**입니다.
> 1. 공분산 행렬 그 자체로는 변수들 간의 상관관계가 섞여 있어 직관적인 파악이 어렵습니다. (예: $x$가 증가할 때 $y$도 적당히 증가한다?)
> 2. 고유값 분해를 하면 이 상관관계를 **"데이터가 가장 넓게 퍼진 주축 방향(고유벡터)"** 과 **"그 방향으로의 크기(고유값)"** 로 깔끔하게 분리해낼 수 있습니다.
> 3. 즉, 복잡한 불확실성 덩어리를 **"장축과 단축을 가진 타원"** 으로 변환하여 눈으로 보고 이해할 수 있게 해주는 도구입니다.
...
### 8. 정규직교(Orthonormal) - 직교 행렬의 핵심 구조

앞서 "직교 행렬에서 R^T = R^(-1)"라고 했는데, 왜 그런지 더 깊이 알아봅시다.

#### 정규직교의 정의

**정규직교(Orthonormal)** 는 두 가지 조건을 **동시에** 만족하는 벡터들의 집합입니다:

| 조건 | 이름 | 의미 | 수식 |
|------|------|------|------|
| 1 | **정규(Normal)** | 각 벡터의 크기가 **1** (단위벡터) | ‖v‖ = 1 |
| 2 | **직교(Ortho)** | 서로 다른 벡터들끼리 **수직** | v₁ · v₂ = 0 |

```
정규 (Normal): "길이가 1인 벡터" = 단위벡터
직교 (Orthogonal): "서로 수직인 벡터들" = 내적이 0
정규직교 (Orthonormal): 둘 다 만족!
```
...
정규직교 벡터들은 **깔끔한 직교 좌표축**을 정의합니다!

#### 회전 행렬의 열벡터 검증

회전 행렬의 열벡터들이 정규직교인지 직접 확인해봅시다:
...
# 열벡터 추출
col1 = R[:, 0]  # 첫 번째 열: [cos30, sin30]
col2 = R[:, 1]  # 두 번째 열: [-sin30, cos30]

# 조건 1: 정규 (크기 = 1)
print(f"첫째 열벡터 크기: {np.linalg.norm(col1):.4f}")  # 1.0000
print(f"둘째 열벡터 크기: {np.linalg.norm(col2):.4f}")  # 1.0000

# 조건 2: 직교 (내적 = 0)
print(f"두 열벡터의 내적: {np.dot(col1, col2):.10f}")   # 0.0000000000
```
...
```
정규: ‖c_i‖ = 1  →  c_i · c_i = 1  →  대각 원소 = 1
직교: c_i ⊥ c_j (i≠j)  →  c_i · c_j = 0  →  비대각 원소 = 0
...
Rz = rotation_z(np.radians(45))

# 세 열벡터
c1, c2, c3 = Rz[:, 0], Rz[:, 1], Rz[:, 2]

# 모든 크기가 1
print(f"‖c1‖ = {np.linalg.norm(c1):.4f}")  # 1.0
print(f"‖c2‖ = {np.linalg.norm(c2):.4f}")  # 1.0
print(f"‖c3‖ = {np.linalg.norm(c3):.4f}")  # 1.0

# 모든 쌍의 내적이 0
print(f"c1·c2 = {np.dot(c1, c2):.10f}")   # 0.0
print(f"c1·c3 = {np.dot(c1, c3):.10f}")   # 0.0
print(f"c2·c3 = {np.dot(c2, c3):.10f}")   # 0.0
```
...
| 분야 | 활용 |
|------|------|
| **SLAM** | 회전 행렬 R의 역변환을 빠르게 계산 (전치만 하면 됨) |
| **좌표계 변환** | 월드↔카메라 변환을 효율적으로 수행 |
| **수치 안정성** | 정규직교 기저는 조건수가 1로 안정적 |
| **PCA** | 공분산 행렬의 고유벡터들이 정규직교 |
| **QR 분해** | 임의 행렬을 정규직교 행렬 Q로 분해 |
...
| 용어 | 정의 | 예시 |
|------|------|------|
| **직교(Orthogonal) 벡터** | 서로 수직 (내적 = 0) | [1,0]과 [0,1] |
| **정규(Normal) 벡터** | 길이가 1인 단위벡터 | [1/√2, 1/√2] |
| **정규직교(Orthonormal)** | 직교 + 정규 | 회전 행렬의 열벡터들 |
| **직교 행렬** | 열벡터들이 정규직교인 행렬 | 회전 행렬 R |

> [!IMPORTANT]
> "직교 행렬(Orthogonal Matrix)"의 열벡터들은 단순히 "직교"만 하는 게 아니라 **"정규직교"**입니다!
> 이름이 헷갈릴 수 있으니 주의하세요.
...
- [x] NumPy로 행렬 곱셈, 역행렬, 고유값을 계산할 수 있다
- [x] 행렬 곱셈이 "선형 변환의 합성"임을 설명할 수 있다
- [x] 고유벡터가 "방향이 변하지 않는 벡터"임을 이해했다
- [x] 2D 회전 행렬을 직접 유도할 수 있다
- [x] 회전 행렬이 직교 행렬인 이유를 기하학적으로 설명할 수 있다
- [x] det(R) = 1인 이유를 "크기 보존 + 방향 유지"로 설명할 수 있다
- [x] 공분산 행렬의 대각 원소가 각 축의 분산임을 안다
- [x] 공분산 행렬의 고유값 분해가 불확실성 타원과 연결됨을 이해했다
