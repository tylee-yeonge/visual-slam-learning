"""
Phase 1 - Week 5: 강체 변환 (SE(3)) 실습
=========================================
SE(3) 변환 행렬과 동차 좌표 실습

README.md의 Part 1~5 내용을 코드로 실습합니다:
- Part 1: SE(3) 변환 행렬
- Part 2: 동차 좌표
- Part 3: 변환 연산 (합성, 역변환)
- Part 4: 좌표계 변환
- Part 5: SLAM 활용

학습 목표:
1. SE(3) 변환 행렬이 무엇인지 이해한다
2. 동차 좌표가 왜 필요한지 체험한다
3. 역변환 공식이 어떻게 유도되는지 확인한다
4. 좌표계 변환과 상대 포즈를 직접 계산해본다
"""

import numpy as np
np.set_printoptions(precision=4, suppress=True)

print("=" * 60)
print("Phase 1 - Week 5: 강체 변환 (SE(3)) 실습")
print("=" * 60)

# ============================================================
# Part 1: SE(3) 변환 행렬
# ============================================================
print("\n" + "=" * 60)
print("Part 1: SE(3) 변환 행렬")
print("=" * 60)

print("""
💡 SE(3)란?
   - Special Euclidean Group (3D)
   - 3D 공간에서 가능한 모든 (회전 + 평행이동) 조합
   - 4×4 행렬로 표현: T = [R|t; 0|1]
""")

# ----- 1.1 기본 함수 정의 -----

# 각 축 회전 함수들
def rotation_x(theta):
    """X축 회전 행렬 생성 (roll)"""
    c, s = np.cos(theta), np.sin(theta)
    return np.array([
        [1,  0,  0],
        [0,  c, -s],
        [0,  s,  c]
    ])

def rotation_y(theta):
    """Y축 회전 행렬 생성 (pitch)"""
    c, s = np.cos(theta), np.sin(theta)
    return np.array([
        [ c,  0,  s],
        [ 0,  1,  0],
        [-s,  0,  c]
    ])

def rotation_z(theta):
    """Z축 회전 행렬 생성 (yaw)"""
    c, s = np.cos(theta), np.sin(theta)
    return np.array([
        [c, -s, 0],
        [s,  c, 0],
        [0,  0, 1]
    ])

def rotation_euler(roll, pitch, yaw):
    """오일러 각으로 복합 회전 행렬 생성
    
    순서: Z-Y-X (yaw-pitch-roll)
    R = Rz(yaw) @ Ry(pitch) @ Rx(roll)
    """
    return rotation_z(yaw) @ rotation_y(pitch) @ rotation_x(roll)


def make_se3(R, t):
    """SE(3) 변환 행렬 생성
    
    T = [ R  | t ]  (4×4)
        [----+---]
        [ 0  | 1 ]
    """
    T = np.eye(4)
    T[:3, :3] = R  # 좌측 상단: 회전 행렬
    T[:3, 3] = t   # 우측 상단: 평행이동 벡터
    return T

def decompose_se3(T):
    """SE(3)에서 R과 t 추출"""
    R = T[:3, :3]
    t = T[:3, 3]
    return R, t

# ----- 1.2 SE(3) 행렬 만들기 -----
print("\n----- 1.2 SE(3) 행렬 만들기 -----")

# 예제: Z축 45도 회전 + (2, 1, 0) 이동
theta = np.radians(45)
R = rotation_z(theta)
t = np.array([2, 1, 0])
T = make_se3(R, t)

print("\n① 회전 행렬 R (Z축 45도 회전):")
print(R)
print(f"\n② 평행이동 벡터 t: {t}")
print("\n③ SE(3) 변환 행렬 T:")
print(T)

print("\n💡 T의 구조 확인:")
print(f"   T[:3, :3] = R (회전 행렬)")
print(f"   T[:3, 3]  = t (평행이동 벡터)")  
print(f"   T[3, :]   = [0, 0, 0, 1] (동차 좌표 유지)")

# ----- 1.3 SE(3)의 자유도 -----
print("\n----- 1.3 SE(3)의 자유도 (6-DOF) -----")
print("""
   자유도 = 독립적인 변수의 개수
   
   | 요소 | 자유도 | 설명 |
   |------|--------|------|
   | 회전 R | 3 | roll, pitch, yaw |
   | 이동 t | 3 | x, y, z |
   | 총합   | 6 | 6-DOF 포즈 |
""")

# ----- 1.4 복합 회전 예제 -----
print("\n----- 1.4 복합 회전 (multiple axes) -----")

print("""
💡 실제 SLAM에서는 모든 축의 회전을 사용합니다!
   - Roll (X축): 카메라가 옆으로 기울어짐
   - Pitch (Y축): 카메라가 위아래로 기울어짐  
   - Yaw (Z축): 카메라가 좌우로 회전
""")

# 각 축 개별 회전 예제
print("\n[개별 축 회전 예제]")
R_x = rotation_x(np.radians(30))
print(f"X축 30도 회전:\n{R_x}\n")

R_y = rotation_y(np.radians(20))
print(f"Y축 20도 회전:\n{R_y}\n")

R_z = rotation_z(np.radians(45))
print(f"Z축 45도 회전:\n{R_z}\n")

# 복합 회전 예제 (Euler angles)
print("[복합 회전: Roll=10°, Pitch=15°, Yaw=30°]")
roll = np.radians(10)
pitch = np.radians(15)
yaw = np.radians(30)

R_euler = rotation_euler(roll, pitch, yaw)
print(f"복합 회전 행렬:\n{R_euler}\n")

# SE(3) 변환에 복합 회전 사용
t_complex = np.array([1, 2, 3])
T_complex = make_se3(R_euler, t_complex)
print(f"복합 회전 + 평행이동 SE(3):\n{T_complex}\n")

print("💡 어떤 회전 행렬이든 SE(3)의 R로 사용 가능!")


# ============================================================
# Part 2: 동차 좌표 (Homogeneous Coordinates)
# ============================================================
print("\n" + "=" * 60)
print("Part 2: 동차 좌표")
print("=" * 60)

print("""
💡 동차 좌표란?
   - 3D 점 (x, y, z) → 동차 좌표 (x, y, z, 1)
   - 마지막에 1을 추가!
   - 이렇게 하면 회전+이동을 행렬 곱 하나로 처리 가능
""")

def to_homogeneous(p):
    """3D 점 → 4D 동차 좌표"""
    return np.append(p, 1)

def from_homogeneous(p_h):
    """4D 동차 좌표 → 3D 점"""
    return p_h[:3] / p_h[3]

# ----- 2.1 일반 좌표 vs 동차 좌표 -----
print("\n----- 2.1 두 가지 변환 방법 비교 -----")

p = np.array([1, 0, 0])  # 테스트 점
print(f"원점 p = {p}")

# 방법 1: 일반 좌표 (덧셈 필요)
p_result1 = R @ p + t
print(f"\n[방법 1] 일반 좌표: p' = R @ p + t")
print(f"         결과: {p_result1}")

# 방법 2: 동차 좌표 (행렬 곱만!)
p_h = to_homogeneous(p)
p_h_result = T @ p_h
p_result2 = from_homogeneous(p_h_result)
print(f"\n[방법 2] 동차 좌표: p' = T @ p_h")
print(f"         p_h = {p_h}")
print(f"         T @ p_h = {p_h_result}")
print(f"         결과: {p_result2}")

print(f"\n✅ 결과 동일: {np.allclose(p_result1, p_result2)}")

# ----- 2.2 동차 좌표의 장점 -----
print("\n----- 2.2 동차 좌표의 장점 -----")
print("""
   ① 회전 + 이동을 행렬 곱 하나로 처리
   ② 연속 변환이 행렬 곱 체인으로 간단화
   ③ 카메라 투영도 행렬 곱으로 표현 가능
   ④ 무한원점(방향 벡터)을 w=0으로 표현 가능
""")

# ============================================================
# Part 3: 변환 연산
# ============================================================
print("\n" + "=" * 60)
print("Part 3: 변환 연산 (합성과 역변환)")
print("=" * 60)

# ----- 3.1 변환 합성 -----
print("\n----- 3.1 변환 합성 -----")

# 변환 1: Z축 90도 회전만
T1 = make_se3(rotation_z(np.radians(90)), np.array([0, 0, 0]))

# 변환 2: X방향 2 이동만
T2 = make_se3(np.eye(3), np.array([2, 0, 0]))

# 합성: 먼저 T1, 다음 T2 적용
T_combined = T2 @ T1

print("T1: Z축 90도 회전")
print("T2: X방향 2 이동")
print("\n⚠️ 주의: 먼저 T1, 다음 T2 적용 = T2 @ T1")
print("        (오른쪽에서 왼쪽으로 읽음!)")

# 테스트
p_test = np.array([1, 0, 0, 1])
print(f"\n점 (1, 0, 0)에 각 변환 적용:")
print(f"  T1만 적용 후:    {(T1 @ p_test)[:3]}")
print(f"  T2 @ T1 적용 후: {(T_combined @ p_test)[:3]}")

# 순서가 바뀌면 결과도 다름!
T_reverse = T1 @ T2
print(f"\n✅ T2 @ T1 결과: {(T_combined @ p_test)[:3]}")
print(f"❌ T1 @ T2 결과: {(T_reverse @ p_test)[:3]} (다른 결과!)")

# ----- 3.2 역변환 -----
print("\n----- 3.2 역변환 -----")

def inverse_se3(T):
    """SE(3) 역변환
    
    T⁻¹ = [ R^T  | -R^T @ t ]
          [------+----------]
          [  0   |    1     ]
    
    이유:
    1. R의 역행렬 = R^T (직교 행렬이므로)
    2. t의 역변환 = -R^T @ t (단순히 -t가 아님!)
    """
    R = T[:3, :3]
    t = T[:3, 3]
    
    T_inv = np.eye(4)
    T_inv[:3, :3] = R.T           # R의 transpose
    T_inv[:3, 3] = -R.T @ t       # 핵심: -R^T @ t
    return T_inv

print("""
💡 SE(3) 역변환 공식:

   T = [R|t]  →  T⁻¹ = [R^T | -R^T·t]
   
   Q: 왜 단순히 -t가 아닌가?
   A: 순변환이 "회전 후 이동"이므로
      역변환은 "이동 취소 후 역회전"
      → 이동 취소도 회전의 영향을 받음!
""")

print("\n원본 T:")
print(T)

T_inv = inverse_se3(T)
print("\nT의 역변환 T⁻¹:")
print(T_inv)

# 검증: T @ T⁻¹ = I
identity = T @ T_inv
print("\n검증: T @ T⁻¹ (단위행렬이어야 함):")
print(identity)
print(f"✅ 단위행렬 맞음: {np.allclose(identity, np.eye(4))}")

# NumPy 역행렬과 비교
T_inv_numpy = np.linalg.inv(T)
print(f"\n✅ NumPy inv와 결과 동일: {np.allclose(T_inv, T_inv_numpy)}")

# ----- 3.3 역변환 상세 예시 -----
print("\n----- 3.3 왜 -R^T·t인가? (구체적 예시) -----")

# 90도 Z축 회전 + X방향 3 이동
theta_90 = np.pi / 2
R_90 = rotation_z(theta_90)
t_90 = np.array([3, 0, 0])

print(f"원본 변환: 90도 회전 + X방향 3 이동")
print(f"  t = {t_90}")
print(f"\n역변환에서:")
print(f"  -t = {-t_90}")
print(f"  -R^T @ t = {-R_90.T @ t_90}")
print("""
💡 결과 해석:
   - 단순히 -t 하면: X방향 -3 이동
   - 올바른 -R^T·t: Y방향 -3 이동
   
   → 왜? 90도 회전된 좌표계에서 X는 원래 좌표계의 Y!
""")

# ============================================================
# Part 4: 좌표계 변환  
# ============================================================
print("\n" + "=" * 60)
print("Part 4: 좌표계 변환")
print("=" * 60)

print("""
💡 핵심 개념:
   - 같은 점이라도 좌표계에 따라 좌표값이 다름
   - T_ab: b 좌표계의 점을 a 좌표계로 변환
   - T_ab @ p_b = p_a (아래첨자가 상쇄됨!)
""")

# ----- 4.1 월드 ↔ 카메라 변환 -----
print("\n----- 4.1 월드 ↔ 카메라 변환 -----")

# 카메라 포즈: 월드에서 (2, 0, 0) 위치, 90도 왼쪽 회전
R_wc = rotation_z(np.radians(90))
t_wc = np.array([2, 0, 0])
T_wc = make_se3(R_wc, t_wc)  # 카메라의 포즈 (월드 기준)

print("T_wc: 카메라 포즈 (월드 좌표계 기준)")
print(f"  위치: (2, 0, 0)")
print(f"  방향: Z축 90도 회전 (왼쪽을 봄)")

# 월드 → 카메라 변환
T_cw = inverse_se3(T_wc)  # 역변환!

print("\nT_cw = T_wc⁻¹: 월드→카메라 변환")

# 월드 좌표의 점을 카메라 좌표로
p_world = np.array([5, 0, 0, 1])
p_camera = T_cw @ p_world

print(f"\n월드 좌표 점: {p_world[:3]}")
print(f"카메라 좌표 점: {p_camera[:3]}")

print("""
💡 결과 해석:
   월드에서 (5, 0, 0)에 있는 점이
   카메라(90도 왼쪽을 봄) 입장에서는
   카메라 앞쪽(Y 방향)에 보임!
""")

# ----- 4.2 상대 포즈 -----  
print("\n----- 4.2 상대 포즈 -----")

print("""
💡 상대 포즈란?
   - 한 좌표계에서 본 다른 좌표계의 위치/방향
   - T_ij = T_wi⁻¹ @ T_wj (i 기준으로 j가 어디?)
   - 아래첨자 팁: iw @ wj = ij (w가 상쇄!)
""")

# 키프레임 1: 원점
T_w1 = make_se3(np.eye(3), np.array([0, 0, 0]))

# 키프레임 2: X방향 2m 이동 + 30도 회전
T_w2 = make_se3(rotation_z(np.radians(30)), np.array([2, 0, 0]))

print("프레임 1: 원점, 회전 없음")
print("프레임 2: (2, 0, 0), Z축 30도 회전")

# 상대 포즈 계산
T_12 = inverse_se3(T_w1) @ T_w2  # 1 기준으로 2가 어디?
T_21 = inverse_se3(T_w2) @ T_w1  # 2 기준으로 1이 어디?

print(f"\n상대 포즈 T_12 (1 → 2):")
print(f"  상대 위치: {T_12[:3, 3]}")
print(f"  (프레임 1 기준으로, 프레임 2는 X방향 2, 30도 회전)")

print(f"\n상대 포즈 T_21 (2 → 1):")
print(f"  상대 위치: {T_21[:3, 3]}")

# 역관계 확인
T_21_from_inv = inverse_se3(T_12)
print(f"\n✅ T_21 = T_12⁻¹ 확인: {np.allclose(T_21, T_21_from_inv)}")

# ============================================================
# Part 5: SLAM 활용
# ============================================================
print("\n" + "=" * 60)
print("Part 5: SLAM에서의 활용")
print("=" * 60)

# ----- 5.1 ROS TF2 연결 -----
print("\n----- 5.1 ROS TF2 연결 -----")

def rotation_to_quaternion(R):
    """회전 행렬 → 쿼터니언 [w, x, y, z]"""
    tr = np.trace(R)
    if tr > 0:
        s = 0.5 / np.sqrt(tr + 1)
        w = 0.25 / s
        x = (R[2, 1] - R[1, 2]) * s
        y = (R[0, 2] - R[2, 0]) * s
        z = (R[1, 0] - R[0, 1]) * s
    else:
        w, x, y, z = 1, 0, 0, 0
    q = np.array([w, x, y, z])
    return q / np.linalg.norm(q)

def se3_to_ros_transform(T):
    """SE(3) → ROS geometry_msgs/Transform 형식"""
    t = T[:3, 3]
    q = rotation_to_quaternion(T[:3, :3])
    
    return {
        'translation': {'x': t[0], 'y': t[1], 'z': t[2]},
        'rotation': {'x': q[1], 'y': q[2], 'z': q[3], 'w': q[0]}  # ROS는 [x,y,z,w] 순서!
    }

ros_tf = se3_to_ros_transform(T_w2)
print("\nT_w2를 ROS Transform 형식으로 변환:")
print(f"  translation: x={ros_tf['translation']['x']:.2f}, "
      f"y={ros_tf['translation']['y']:.2f}, z={ros_tf['translation']['z']:.2f}")
print(f"  rotation: x={ros_tf['rotation']['x']:.4f}, "
      f"y={ros_tf['rotation']['y']:.4f}, z={ros_tf['rotation']['z']:.4f}, "
      f"w={ros_tf['rotation']['w']:.4f}")

print("""
💡 ROS TF2 구조:
   - translation: Vector3 (x, y, z)
   - rotation: Quaternion (x, y, z, w)
   - ⚠️ 주의: ROS 쿼터니언은 [x, y, z, w] 순서!
""")

# ============================================================
# 정리
# ============================================================
print("\n" + "=" * 60)
print("📝 Week 5 핵심 정리")
print("=" * 60)
print("""
┌─────────────────────────────────────────────────────────┐
│ Part 1: SE(3) 변환 행렬                                  │
│   • 4×4 행렬: T = [R|t; 0|1]                            │
│   • 6 자유도 (회전 3 + 이동 3)                           │
├─────────────────────────────────────────────────────────┤
│ Part 2: 동차 좌표                                        │
│   • (x, y, z) → (x, y, z, 1)                            │
│   • 회전+이동을 행렬 곱 하나로!                          │
├─────────────────────────────────────────────────────────┤
│ Part 3: 변환 연산                                        │
│   • 합성: T2 @ T1 (순서 중요!)                          │
│   • 역변환: T⁻¹ = [R^T | -R^T·t]                        │
├─────────────────────────────────────────────────────────┤
│ Part 4: 좌표계 변환                                      │
│   • T_ab @ p_b = p_a (아래첨자 상쇄)                     │
│   • 상대 포즈: T_ij = T_wi⁻¹ @ T_wj                     │
├─────────────────────────────────────────────────────────┤
│ Part 5: SLAM 활용                                        │
│   • 카메라 포즈: T_wc                                    │
│   • ROS TF2: translation + rotation(쿼터니언)           │
└─────────────────────────────────────────────────────────┘

🎯 다음 단계: se3_quiz.py로 개념 확인 → Week 6: Lie 군/대수
""")
